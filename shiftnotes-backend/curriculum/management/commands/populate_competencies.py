...
from curriculum.models import CoreCompetency, SubCompetency
import uuid


class Command(BaseCommand):
    help = 'Populate core competencies and sub-competencies for a program from milestones data'

    def add_arguments(self, parser):
        parser.add_argument(
            'program_id',
            type=str,
            help='UUID of the program to populate competencies for'
        )
        parser.add_argument(
            '--clear-existing',
            action='store_true',
            help='Clear existing competencies for this program before populating'
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Populating competencies for program: {program.name}')

        # Clear existing competencies if requested
        if clear_existing:
            self.stdout.write('Clearing existing competencies...')
            SubCompetency.objects.filter(program=program).delete()
            CoreCompetency.objects.filter(program=program).delete()
            self.stdout.write('Existing competencies cleared.')

        # Milestone data from the TypeScript file
        competencies_data = [
            {
                'id': 'PC',
                'name': 'Patient Care',
                'subCompetencies': [
                    {
                        ...
                        'levels': [
                            {'level': 1, 'descriptions': ['Detects when a patient\'s vital signs are abnormal']},
                    ...
                    },
                    {
                        ...
                        'levels': [
                            {'level': 1, 'descriptions': ['Elicits and communicates a reliable comprehensive patient history and performs a physical exam']},
                            {'level': 2, 'descriptions': ['Elicits and communicates a focused patient history and performs a focused physical exam that effectively address the patient\'s chief complaint and urgent issues']},
                            {'level': 3, 'descriptions': ['Prioritizes essential components of a patient history and physical exam, given a limited or dynamic circumstance']},
                            {'level': 4, 'descriptions': ['Using all potential sources of data, gathers those that are necessary for the beneficial management of patients']},
                            {'level': 5, 'descriptions': ['Models the effective use of a patient history and physical exam to minimize the need for further diagnostic testing']}
                        ]
                    },
                    {
                        'id': 'PC3',
                    ...
                    },
                    {
                        ...
                        'name': 'Diagnosis',
                        'levels': [
                            {'level': 1, 'descriptions': ['Constructs a list of potential diagnoses based on the patient\'s chief complaint and initial assessment']},
                            {'level': 2, 'descriptions': ['Provides a prioritized differential diagnosis']},
                            {'level': 3, 'descriptions': ['Provides a diagnosis for common medical conditions and demonstrates the ability to modify a diagnosis based on a patient\'s clinical course and additional data']},
                            {'level': 4, 'descriptions': ['Provides a diagnosis for patients with multiple comorbidities or uncommon medical conditions, recognizing errors in clinical reasoning']},
                            {'level': 5, 'descriptions': ['Serves as a role model and educator to other learners for deriving diagnoses and recognizing errors in clinical reasoning']}
                        ]
                    },
                    {
                        'id': 'PC5',
                        ...
                        'levels': [
                            ...
                            {'level': 5, 'descriptions': ['Participates in developing departmental and/or institutional policies on pharmacy and therapeutics']}
                        ]
                    },
                    {
                        'id': 'PC6',
                    ...
                    },
                    {
                        ...
                        'levels': [
                            {'level': 1, 'descriptions': ['Manages a single patient amidst distractions']},
                            {'level': 2, 'descriptions': ['Task-switches between different patients of similar acuity']},
                            {'level': 3, 'descriptions': ['Employs task-switching in an efficient manner to manage multiple patients of varying acuity and at varying stages of work-up']},
                            {'level': 4, 'descriptions': ['Employs task-switching in an efficient manner to manage the emergency department']},
                            {'level': 5, 'descriptions': ['Employs task switching in an efficient manner to manage the emergency department under high-volume or surge situations']}
                        ]
                    },
                    {
                        'id': 'PC8',
                ...
                'subCompetencies': [
                    {
                        ...
                        'levels': [
                            {'level': 1, 'descriptions': ['Demonstrates scientific knowledge of common presentations and conditions']},
                            {'level': 2, 'descriptions': ['Demonstrates scientific knowledge of complex presentations and conditions']},
                            {'level': 3, 'descriptions': ['Integrates scientific knowledge of comorbid conditions for complex presentations']},
                            {'level': 4, 'descriptions': ['Integrates scientific knowledge of uncommon, atypical, or complex comorbid conditions for complex presentations']},
                            {'level': 5, 'descriptions': ['Pursues and integrates new and emerging knowledge']}
                        ]
                    },
                    {
                        'id': 'MK2',
            ...
            },
            {
                ...
                'subCompetencies': [
                    ...
                    },
                    {
                        ...
                        'levels': [
                            {'level': 1, 'descriptions': ['Demonstrates knowledge of basic quality improvement methodologies and metrics']},
                            {'level': 2, 'descriptions': ['Describes local quality improvement initiatives (e.g., emergency department throughput, testing turnaround times)']},
                            {'level': 3, 'descriptions': ['Participates in local quality improvement initiatives']},
                            {'level': 4, 'descriptions': ['Demonstrates the skills required for identifying, developing, implementing, and analyzing a quality improvement project']},
                            {'level': 5, 'descriptions': ['Creates, implements, and assesses quality improvement initiatives at the institutional or community level']}
                        ]
                    },
                    {
                        'id': 'SBP3',
                ...
                'subCompetencies': [
                    {
                        ...
                        'levels': [
                            {'level': 1, 'descriptions': ['Demonstrates how to access and use available evidence']},
                            {'level': 2, 'descriptions': ['Articulates the clinical questions that are necessary to guide evidence-based care']},
                            {'level': 3, 'descriptions': ['Locates and applies the best available evidence, integrating it with patient preference, to the care of complex patients']},
                            {'level': 4, 'descriptions': ['Critically appraises and applies evidence even in the face of uncertainty and of conflicting evidence to guide care that is tailored to the individual patient']},
                            {'level': 5, 'descriptions': ['Coaches others to critically appraise and apply evidence for complex patients, and/or participates in the development of guidelines']}
                        ]
                    },
                    {
                        'id': 'PBLI2',
                        ...
                        'levels': [
                            {'level': 1, 'descriptions': ['Demonstrates an openness to performance data (feedback and other input)']},
                            {'level': 2, 'descriptions': [
                ...
                'subCompetencies': [
                    ...
                    },
                    {
                        ...
                        'levels': [
                            ...
                            {'level': 5, 'descriptions': ['Takes ownership of system outcomes']}
                        ]
                    },
                    {
                        'id': 'PROF3',
                        ...
                        'levels': [
                            {'level': 1, 'descriptions': ['Recognizes, with assistance, the status of one\'s personal and professional well-being']},
                            {'level': 2, 'descriptions': ['Independently recognizes the status of one\'s personal and professional well-being and engages in help-seeking behaviors']},
                            {'level': 3, 'descriptions': ['With assistance, proposes a plan to optimize personal and professional well-being']},
                            {'level': 4, 'descriptions': ['Independently develops a plan to optimize one\'s personal and professional well-being']},
                            {'level': 5, 'descriptions': ['Coaches others when their emotional responses or level of knowledge/skills fail to meet professional expectations']}
                        ]
                    }
                ]
            },
        ...
        with transaction.atomic():
            ...
            created_sub_competencies = 0

            for comp_data in competencies_data:
                # Create core competency
                core_comp, created = CoreCompetency.objects.get_or_create(
                    program=program,
                    code=comp_data['id'],
                    defaults={'title': comp_data['name']}
                )
                
                if created:
                    created_competencies += 1
                    self.stdout.write(f'Created core competency: {core_comp.code} - {core_comp.title}')
                else:
                    self.stdout.write(f'Core competency already exists: {core_comp.code} - {core_comp.title}')

                # Create sub-competencies
                for sub_comp_data in comp_data['subCompetencies']:
                    # Convert milestone levels to text fields
                    milestone_texts = {}
                    for level_data in sub_comp_data['levels']:
                        level_num = level_data['level']
                        descriptions = level_data['descriptions']
                        # Join multiple descriptions with semicolons
                        milestone_texts[f'milestone_level_{level_num}'] = '; '.join(descriptions)

                    sub_comp, created = SubCompetency.objects.get_or_create(
                        program=program,
                        core_competency=core_comp,
                        code=sub_comp_data['id'],
                        defaults={
                            'title': sub_comp_data['name'],
                            **milestone_texts
                        }
                    )
                    
                    if created:
                        created_sub_competencies += 1
                        self.stdout.write(f'Created sub-competency: {sub_comp.code} - {sub_comp.title}')
                    else:
                        self.stdout.write(f'Sub-competency already exists: {sub_comp.code} - {sub_comp.title}')

        self.stdout.write(
            self.style.SUCCESS(
                f'Successfully populated competencies for program {program.name}:\n'
                f'- {created_competencies} core competencies created\n'
                f'- {created_sub_competencies} sub-competencies created'
            )
        )
